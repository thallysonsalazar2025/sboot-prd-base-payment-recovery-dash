...
public class PaymentService {
    ...
    public PaymentDtos.PaymentStatusResponse get(Long id) {
        return mapper.toStatusResponse(repository.findById(id).orElseThrow(() -> new NotFoundException("Payment not found")));
    }

    public PaymentDtos.PaymentMethodsResponse methods() {
        return new PaymentDtos.PaymentMethodsResponse(client.getPaymentMethods());
    }
    ...
    public PaymentDtos.CreatePaymentResponse retry(Long id) {
        ...
        if (tx.getStatus() != PaymentStatus.REJECTED)
            throw new IllegalStateException("Retry allowed only for rejected status");
        var ext = client.retryPayment(tx.getExternalId());
        tx.setStatus(ext.status());
        repository.save(tx);
        return new PaymentDtos.CreatePaymentResponse(ext.id(), ext.checkoutUrl(), ext.qrCode(), ext.status());
    }
}
